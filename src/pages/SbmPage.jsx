...
import { sbmCurated } from '../data/sbmData';
import { SEO } from '../components/shared';
import ProductDocumentation from '../components/ProductDocumentation';
import { useLanguage } from '../context/LanguageContext';

// Paleta oficial SBM (muestreada de sbm.fr): azul logo #0070B8, azul corporativo
// #273867, amarillo #F8F000 y rojo #E81820 del logotipo. Combinada con acentos 3P.
const SBM = {
  dark: '#273867',
  blue: '#0070B8',
  accent: '#F8F000',
...
const description = {
  es: 'Criadoras (éleveuses) de infrarrojo cerámico y control VisioLON/Sunmaster para avicultura. SBM, fabricante francés desde 1946, 100% hecho en Clénay, Borgoña.',
  en: 'Ceramic infrared gas brooders (éleveuses) and VisioLON/Sunmaster control for poultry. SBM, French manufacturer since 1946, 100% made in Clénay, Burgundy.',
};

const whatsappProductUrl = (producto) => {
  const text = encodeURIComponent(
    `Hola, me interesa cotizar el producto SBM: ${producto.codigo} - ${producto.descripcion}`
  );
...
const SbmPage = () => {
  ...
  const productos = useMemo(() => {
    const catalog = brandCatalogs['sbm'];
    if (!catalog) return [];
    return catalog.productos.map((p) => {
      const c = sbmCurated.productos[p.codigo];
  ...
  const categoriaDe = (id) =>
    sbmCurated.categorias.find((c) => c.id === id) || null;

  return (
    <div className="min-h-screen" style={{ backgroundColor: '#f2f5fa' }}>
      <SEO
        title="SBM | Calefacción radiante para avicultura - 3P S.A. DE C.V."
        description={description.es}
        keywords="SBM, criadoras gas, calefacción radiante cerámica, brooders avícolas, VisioLON, Sunmaster, éleveuses, calefacción granja"
      />

      {/* Hero — identidad SBM (azul corporativo) con acentos 3P */}
      <section
        className="relative text-white overflow-hidden"
        style={{ backgroundColor: SBM.dark }}
      >
        {/* Glows decorativos */}
        <div
          className="absolute -top-32 -right-32 w-96 h-96 rounded-full pointer-events-none"
          style={{ backgroundColor: SBM.accent, opacity: 0.12, filter: 'blur(80px)' }}
        />
        <div
          className="absolute -bottom-40 -left-24 w-80 h-80 rounded-full pointer-events-none"
          style={{ backgroundColor: SBM.blue, opacity: 0.35, filter: 'blur(70px)' }}
        />
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-10">
            ...
            <div className="max-w-2xl">
              ...
              <h1 className="text-3xl md:text-4xl font-bold mb-4 leading-tight">
                Calefacción radiante cerámica a gas
              </h1>
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-white/70">
                <span className="flex items-center gap-2">
                  <Package size={16} style={{ color: SBM.accent }} />
                  {productos.length} productos
                </span>
                <span className="flex items-center gap-2">
                  <LayoutGrid size={16} style={{ color: SBM.accent }} />
                  {sbmCurated.categorias.length} categorías
                </span>
                <span className="flex items-center gap-2">
                  <MapPin size={16} style={{ color: SBM.accent }} />
                  León, Guanajuato
                </span>
              </div>
            </div>
        ...
        {/* Franja de acento degradado SBM -> 3P */}
        <div
          className="h-1.5 w-full"
          style={{
            background: `linear-gradient(90deg, ${SBM.accent} 0%, #E81820 60%, #C41E3A 100%)`,
          }}
        />
      </section>
      ...
      <section
        ...
        style={{ backgroundColor: SBM.blue, top: 'var(--header-h, 136px)' }}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              style={
                categoriaActiva === 'todos' ? { backgroundColor: SBM.accent, color: SBM.dark } : {}
              }
            >
              <LayoutGrid size={15} />
              Todos
            ...
            {sbmCurated.categorias.map((cat) => {
              ...
              return (
                <button
                  ...
                  style={activa ? { backgroundColor: SBM.accent, color: SBM.dark } : {}}
                >
                  <Icon size={15} />
                  {cat[language] || cat.es}
          ...
          <div className="flex justify-end mt-3">
            <div className="relative w-full sm:w-80">
              ...
              <input
                ...
                style={{ '--tw-ring-color': SBM.accent }}
              />
            </div>
          </div>
        </div>
          ...
          <p className="text-sm text-gray-500 mb-6">
            ...
            {categoriaActiva !== 'todos' && (
              <>
                {' '}
                en{' '}
                <span className="font-semibold" style={{ color: SBM.dark }}>
              ...
              >
                <div className="h-48 bg-white flex items-center justify-center p-4 relative overflow-hidden">
                  ...
                  <div
                    ...
                    style={{ backgroundColor: `${SBM.blue}1a` }}
                  >
                    <Package size={40} style={{ color: SBM.blue }} />
                  </div>
                </div>
                <div className="p-4 flex-1 flex flex-col border-t border-gray-50">
                  <span
                    className="text-xs font-semibold mb-2"
                    style={{ color: SBM.dark }}
                  >
                    SKU: {prod.codigo}
                  </span>
        ...
        >
          ...
          >
            ...
            <div className="p-6 md:p-8">
              ...
              <ProductDocumentation marca="sbm" codigo={selectedProduct.codigo} />
      ...
      {/* CTA inferior */}
      <section className="text-white py-16" style={{ backgroundColor: SBM.dark }}>
        <div className="max-w-4xl mx-auto px-4 text-center">
          <img
            src="/images/brands/sbm-oficial.png"
            alt="SBM"
            className="h-14 w-auto mx-auto mb-6 bg-white rounded-xl px-4 py-2"
            loading="lazy"
          />
          <h2 className="text-2xl md:text-3xl font-bold mb-4">
            ¿Necesitas una cotización de productos SBM?
          </h2>
          <p className="text-white/75 mb-8">
            Contamos con inventario y refacciones SBM en León, Guanajuato.
            Escríbenos con el código del producto que necesitas.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a
              href={`https://wa.me/${WHATSAPP_PHONE}?text=${encodeURIComponent(
                'Hola, me interesa cotizar productos SBM.'
              )}`}
              target="_blank"
              rel="noopener noreferrer"
