...
import { amtCurated } from '../data/amtData';
import { SEO } from '../components/shared';
import ProductDocumentation from '../components/ProductDocumentation';
import { useLanguage } from '../context/LanguageContext';

// Paleta AMT (analizada del logo oficial amt-usa.com): rojo gallo #CC3627 +
// negro wordmark. Aproximación oficial (no hay brand kit público).
...
const description = {
  es: 'Malacates, poleas y ajustadores de suspensión para líneas de bebida y alimentación. AMT (Agricultural Mfg. & Textiles, Inc.), Laurens, Carolina del Sur, EE. UU., desde 1975.',
  en: 'Winches, pulleys and suspension adjusters for feed and water lines. AMT (Agricultural Mfg. & Textiles, Inc.), Laurens, South Carolina, USA, since 1975.',
};

const whatsappProductUrl = (producto) => {
  const text = encodeURIComponent(
    `Hola, me interesa cotizar el producto AMT: ${producto.codigo} - ${producto.descripcion}`
  );
...
const AmtPage = () => {
  ...
  const productos = useMemo(() => {
    const catalog = brandCatalogs['amt'];
    if (!catalog) return [];
    return catalog.productos.map((p) => {
      const c = amtCurated.productos[p.codigo];
  ...
  const categoriaDe = (id) =>
    amtCurated.categorias.find((c) => c.id === id) || null;

  const pillActiva = { backgroundColor: '#ffffff', color: AMT.red };

  return (
    <div className="min-h-screen" style={{ backgroundColor: '#f7f5f4' }}>
      <SEO
        title="AMT | Malacates y suspensión para avicultura - 3P S.A. DE C.V."
        description={description.es}
        keywords="AMT, malacate granja, poleas avícolas, winch avicultura, suspensión bebederos, Agricultural Mfg Textiles"
      />

      {/* Hero — identidad AMT (negro + rojo) con acentos 3P */}
      <section
        className="relative text-white overflow-hidden"
        style={{ backgroundColor: AMT.dark }}
      >
        {/* Glows decorativos */}
        <div
          className="absolute -top-32 -right-32 w-96 h-96 rounded-full pointer-events-none"
          style={{ backgroundColor: AMT.red, opacity: 0.25, filter: 'blur(80px)' }}
        />
        <div
          className="absolute -bottom-40 -left-24 w-80 h-80 rounded-full pointer-events-none"
          style={{ backgroundColor: AMT.red, opacity: 0.15, filter: 'blur(70px)' }}
        />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 py-14 md:py-20">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-10">
            {/* Logo AMT grande sobre tarjeta blanca */}
            <div className="flex-shrink-0">
              <div className="bg-white rounded-2xl px-8 py-6 md:px-10 md:py-8 shadow-2xl">
                <img
                  src="/images/brands/amt-oficial.png"
                  alt="AMT — Agricultural Mfg. & Textiles, Inc."
            ...
            <div className="max-w-2xl">
              ...
              <h1 className="text-3xl md:text-4xl font-bold mb-4 leading-tight">
                Suspensión e izaje para avicultura
              </h1>
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-white/70">
                <span className="flex items-center gap-2">
                  <Package size={16} style={{ color: AMT.red }} />
                  {productos.length} productos
                </span>
                <span className="flex items-center gap-2">
                  <LayoutGrid size={16} style={{ color: AMT.red }} />
                  {amtCurated.categorias.length} categorías
                </span>
                <span className="flex items-center gap-2">
                  <MapPin size={16} style={{ color: AMT.red }} />
                  León, Guanajuato
                </span>
              </div>
            </div>
        ...
        {/* Franja de acento degradado AMT -> 3P */}
        <div
          className="h-1.5 w-full"
          style={{
            background: `linear-gradient(90deg, ${AMT.red} 0%, ${AMT.red} 60%, #C41E3A 100%)`,
          }}
        />
      </section>
      ...
      <section
        ...
        style={{ backgroundColor: AMT.red, top: 'var(--header-h, 136px)' }}
      >
          ...
          <p className="text-sm text-gray-500 mb-6">
            ...
            {categoriaActiva !== 'todos' && (
              <>
                {' '}
                en{' '}
                <span className="font-semibold" style={{ color: AMT.red }}>
              ...
              >
                <div className="h-48 bg-white flex items-center justify-center p-4 relative overflow-hidden">
                  ...
                  <div
                    ...
                    style={{ backgroundColor: `${AMT.red}1a` }}
                  >
                    <Package size={40} style={{ color: AMT.red }} />
                  </div>
                </div>
        ...
        >
          ...
          >
            ...
            <div className="p-6 md:p-8">
              ...
              <ProductDocumentation marca="amt" codigo={selectedProduct.codigo} />
      ...
      {/* CTA inferior */}
      <section className="text-white py-16" style={{ backgroundColor: AMT.dark }}>
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <h2 className="text-2xl md:text-3xl font-bold mb-4">
            ¿Necesitas una cotización de productos AMT?
          </h2>
          <p className="text-white/75 mb-8">
            Contamos con inventario y refacciones AMT en León, Guanajuato.
            Escríbenos con el código del producto que necesitas.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a
              href={`https://wa.me/${WHATSAPP_PHONE}?text=${encodeURIComponent(
                'Hola, me interesa cotizar productos AMT.'
              )}`}
              target="_blank"
              rel="noopener noreferrer"
